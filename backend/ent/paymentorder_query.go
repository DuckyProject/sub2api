...
func (_q *PaymentOrderQuery) QueryProduct() *PaymentProductQuery {
...
// QueryGrantGroup chains the current query on the "grant_group" edge.
func (_q *PaymentOrderQuery) QueryGrantGroup() *GroupQuery {
...
func (_q *PaymentOrderQuery) QueryEntitlementEvents() *EntitlementEventQuery {
...
func (_q *PaymentOrderQuery) Clone() *PaymentOrderQuery {
	...
	return &PaymentOrderQuery{
		...
		withGrantGroup:        _q.withGrantGroup.Clone(),
...
func (_q *PaymentOrderQuery) WithProduct(opts ...func(*PaymentProductQuery)) *PaymentOrderQuery {
	query := (&PaymentProductClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithGrantGroup tells the query-builder to eager-load the nodes that are connected to
// the "grant_group" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PaymentOrderQuery) WithGrantGroup(opts ...func(*GroupQuery)) *PaymentOrderQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGrantGroup = query
	return _q
}
...
func (_q *PaymentOrderQuery) WithEntitlementEvents(opts ...func(*EntitlementEventQuery)) *PaymentOrderQuery {
	query := (&EntitlementEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	...
	if query := _q.withProduct; query != nil {
		if err := _q.loadProduct(ctx, query, nodes, nil,
			func(n *PaymentOrder, e *PaymentProduct) { n.Edges.Product = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withGrantGroup; query != nil {
		if err := _q.loadGrantGroup(ctx, query, nodes, nil,
			func(n *PaymentOrder, e *Group) { n.Edges.GrantGroup = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withEntitlementEvents; query != nil {
		if err := _q.loadEntitlementEvents(ctx, query, nodes,
			func(n *PaymentOrder) { n.Edges.EntitlementEvents = []*EntitlementEvent{} },
			func(n *PaymentOrder, e *EntitlementEvent) {
...
func (_q *PaymentOrderQuery) loadProduct(ctx context.Context, query *PaymentProductQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *PaymentProduct)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*PaymentOrder)
	for i := range nodes {
...
func (_q *PaymentOrderQuery) loadGrantGroup(ctx context.Context, query *GroupQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *Group)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*PaymentOrder)
	for i := range nodes {
		if nodes[i].GrantGroupID == nil {
			continue
		}
		fk := *nodes[i].GrantGroupID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "grant_group_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *PaymentOrderQuery) loadEntitlementEvents(ctx context.Context, query *EntitlementEventQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *EntitlementEvent)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(entitlementevent.FieldOrderID)
	}
	query.Where(predicate.EntitlementEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.EntitlementEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *PaymentOrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withProduct != nil {
			_spec.Node.AddColumnOnce(paymentorder.FieldProductID)
		}
		if _q.withGrantGroup != nil {
			_spec.Node.AddColumnOnce(paymentorder.FieldGrantGroupID)
		}
	}
